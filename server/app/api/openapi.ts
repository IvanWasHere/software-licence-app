  ...
  return {
    ...
    'info': {
      ...
      description: [
        ...
        '',
        '**A 402 is a stop signal.** Anything that runs into an account limit (seats, API',
        "keys, this month's calls) answers 402 `plan_limit_exceeded`. Do not retry it: call",
        '`GET /organization` to see `usage.*.remaining`, and treat a 402 as "stop and tell the',
        'customer", not as a failure to back off from.',
      ].join('\n'),
    },
    'servers': [{ url: server }],
