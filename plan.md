...
- `licenses`, with these column groups:
  - Ownership: `organization_id`, `product_id`, `plan_id`, `order_id`, `subscription_id` (null for one-time).
  - Key: `key_hash` (SHA-256, unique, used for lookup), `key_encrypted` (app encryption, so the portal and emails can show it again), `key_last4`.
...
**Key format:** `{PREFIX}-XXXXX-XXXXX-XXXXX-XXXXX`, using the Crockford base32 alphabet (no I/L/O/U), which gives 100 bits of entropy. Input is normalized before hashing: uppercase, dashes and whitespace stripped, prefix kept.
...
- **`ResponseSigner`**
  - Signs license API responses with Ed25519 (key pair in env, public key published at `/api/v1/keys`).
  - SDKs verify the signature before caching, so an edited local cache or a fake local server cannot unlock the product.
  - This is what makes offline grace safe.
...
- Build `ValidationService`, `ActivationService` and the Ed25519 signer.
- Admin license pages with manual issue.
- ✅ Unit-test matrix covers every reason code, and staff can issue a key manually.
